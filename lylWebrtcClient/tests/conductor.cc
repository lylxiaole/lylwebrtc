...
Conductor::Conductor() : peer_id_(-1), loopback_(false)
{
}
 
bool Conductor::Initialize_thread() {
...
bool Conductor::InitializePeerConnection()
{
	...
	worker_thread->Invoke<void>(RTC_FROM_HERE, rtc::Bind(&Conductor::CreateAudioDeviceModule_w, this));
...
void Conductor::CreateAudioDeviceModule_w()
{
...
void Conductor::OnIceCandidate(const webrtc::IceCandidateInterface* candidate) {
	...
	{
		RTC_LOG(LS_ERROR) << "Failed to serialize candidate" << this->peer_id_;
		return;
	}
	jmessage[kCandidateSdpName] = sdp;
	 
	auto xxxx = writer.write(jmessage); 
	SendMessage(xxxx); 
}
...
void Conductor::OnMessageFromPeer(int peer_id, const std::string& message)
{
	...
	{
		RTC_DCHECK(peer_id_ != -1);
		RTC_LOG(WARNING) << "Received a message from unknown peer while already in a conversation with a different peer.";
		return;
	}
	...
	else
	{
		...
		//auto candidatexx = peer_connection_->remote_description(); 
		//if (!candidatexx)
		//{
		//	RTC_LOG(WARNING) << "candidate not ready";
		//}
		cacheCandidates_.push_back(candidate);
...
void Conductor::DealCacheCandidates()
{
...
void Conductor::AddTracks() {
	...
	//			kVideoLabel, peer_connection_factory_->CreateVideoSource(std::move(video_device), nullptr)));
